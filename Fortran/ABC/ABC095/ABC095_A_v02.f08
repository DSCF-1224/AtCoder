...
module ABC095
	...
	subroutine ABC095_A
		...
		val_total = val_base
		if( ichar( buffer(1:1) ) .eq. ichar(chr_added) ) val_total = val_total+val_topped
		if( ichar( buffer(2:2) ) .eq. ichar(chr_added) ) val_total = val_total+val_topped
		if( ichar( buffer(3:3) ) .eq. ichar(chr_added) ) val_total = val_total+val_topped
