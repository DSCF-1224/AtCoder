...
! [status]   https://atcoder.jp/contests/abc125/submissions/5827722 : WA
! [status]   https://atcoder.jp/contests/abc125/submissions/5827765 : AC
...
module ABC125
  ...
  subroutine task_D
    ...
    allocate( dp(1:len_sequence+1, 1:2) )
    ...
    ! dynamic planning
    dp(1,1) = 0_INT64
    dp(1,2) = - sum( abs( val_sequence(1:len_sequence) ) )

    do itr = 1_INT32, len_sequence, 1_INT32
      dp(itr + 1, 1) = max( dp(itr, 1) + val_sequence(itr), dp(itr, 2) - val_sequence(itr) )
      dp(itr + 1, 2) = max( dp(itr, 1) - val_sequence(itr), dp(itr, 2) + val_sequence(itr) )
    end do

    ! STEP.05
    ! output the maximum value of the summation that can be realized in a given sequence
    write(unit=OUTPUT_UNIT, fmt='(I0)', advance='yes') dp(len_sequence + 1, 1)
