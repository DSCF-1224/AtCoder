...
! [status]   https://atcoder.jp/contests/abc134/submissions/6487601 : WA
...
module ABC134
  ...
  subroutine task_C
    ...
    maxval_1st = maxval( val_sequence(:), dim=1 )
    maxval_2nd = maxval( val_sequence(:), dim=1, mask=val_sequence .ne. maxval_1st )

    stat = .false.

    do itr = 1_INT32, len_sequence, 1_INT32
      if ( maxval_2nd .eq. val_sequence(itr) ) then
        stat = .true.
        exit
      end if
    end do
